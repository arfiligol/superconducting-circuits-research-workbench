...
function _d3_objective_authority(authority)
    authority == D3_HUMAN_APPROVED_OBJECTIVE_AUTHORITY || error(
        "D3 objective authority does not equal the Human-accepted revision-7 target contract.",
    )
...
function _d3_objective_slot(slot_hz)
    ...
    isfinite(slot) && slot > 0 ||
        error("D3 objective Slot must be finite and positive.")
    return slot
end
...
function _d3_validate_response_authority(metrics, authority, label)
    for (name, expected) in (
        ...
        Symbol(getproperty(metrics, name)) == expected || error(
            "$(label) $(name) disagrees with the revision-7 authority.",
        )
    end
    return nothing
end
...
Evaluate the single revision-7 Stage-2 Equivalent objective. Matrix-space and
...
Evaluate the independent revision-7 Stage-3 Hybridized objective. Every
