...
include(joinpath(D3_SOURCE_ROOT, "d3_rev10_models.jl"))
include(joinpath(D3_SOURCE_ROOT, "d3_rev10_objective.jl"))
...
function load_manifest(path)
    ...
    String(manifest["sources"]["target_contract_sha256"]) ==
        D3_REV10_OBJECTIVE_AUTHORITY.target_contract_sha256 || error(
            "Search manifest and current Objective bind different target contracts.",
        )
    cma = manifest["cma_es"]
...
function seed_candidate(manifest, slot_hz)
    ...
    candidate = (
        ...
        l_short_m=Float64(raw["l_short_m"]),
        lc_m=Float64(raw["lc_m"]),
        lp_open_m=Float64(raw["lp_open_m"]),
        u_IDC=Float64(raw["u_IDC"]),
    )
    propertynames(candidate) == SEARCH_COORDINATES || error("Seed order is wrong.")
...
function bind_inputs(manifest, q3d_path, idc_path)
    ...
    inputs = bind_d3_direct_hybridized_inputs(
        q2d,
        q3d,
        idc;
...
function build_context(manifest, inputs; refinement_level)
    ...
    plan = d3_direct_hybridized_grid_plan(
        reference,
        inputs;
    ...
    context = build_d3_targeted_schur_objective_context(
        reference,
        inputs;
        grid_plan=plan,
        id="d3-rev10-targeted-schur-N$(1 << refinement_level)",
    )
...
function relative_change(coarse, fine)
    coarse_value = Float64(coarse)
    fine_value = Float64(fine)
    isfinite(coarse_value) && isfinite(fine_value) || error(
        "Winner validation requires finite cared outputs.",
    )
    denominator = abs(coarse_value)
    iszero(denominator) && return iszero(fine_value) ? 0.0 : Inf
    return abs(fine_value - coarse_value) / denominator
end

function json_relative_change(value)
    numeric = Float64(value)
    isfinite(numeric) && return numeric
    numeric == Inf || error("Winner validation produced an invalid relative change.")
    return "Inf"
end
...
function validate_winner(manifest, inputs, slot_hz, winner)
    ...
    numeric_changes = NamedTuple{strict_names}(Tuple(
        relative_change(getproperty(coarse, name), getproperty(fine.cared, name))
        for name in strict_names
    ))
    strict_changes = map(json_relative_change, numeric_changes)
    pass = all(value -> value <= 0.001, values(numeric_changes))
    return (
        status=pass ? "PASS" : "ABOVE_HUMAN_REFINEMENT_LIMIT",
        thresholds=(primary_quantities=0.001,),
        fine=fine,
        relative_changes=strict_changes,
        relative_change_encoding=
            "finite_number_or_string_Inf_for_zero_coarse_to_nonzero_fine",
        pass=pass,
    )
end
...
function run(options)
    ...
    workbench_tracked_dirty = !isempty(command_output(
        "git",
        "status",
        "--porcelain=v1",
        "--untracked-files=no",
    ))
    workbench_tracked_dirty && error(
        "Tracked Workbench source files must be clean for a recorded search run.",
    )
    source = (
        ...
        workbench_tracked_dirty=workbench_tracked_dirty,
