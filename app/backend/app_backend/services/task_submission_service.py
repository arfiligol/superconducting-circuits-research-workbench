...
class TaskSubmissionService:
    ...
    def _validate_characterization_submission(
        ...
        *,
        ...
        if design.lifecycle_state != "active":
            if design.redirect_design_id is not None:
                raise service_error(
                    ...
                    category="conflict",
                    message=(
                        f"Design {setup.design_id} was redirected to {design.redirect_design_id}."
                    ),
                    details={
        ...
        prerequisite_state, prerequisite_requirement = resolve_characterization_prerequisite_state(
            spec=spec,
        ...
        if (
            ...
            and len(
                {_characterization_input_axis_structure_key(detail) for detail in trace_details}
            )
            > 1
        ):
            raise service_error(
...
def _characterization_input_axis_structure_key(
    ...
    sweep_axes = tuple(axis for axis in detail.axes if axis.name != "frequency" and axis.length > 1)
    if len(sweep_axes) == 0:
