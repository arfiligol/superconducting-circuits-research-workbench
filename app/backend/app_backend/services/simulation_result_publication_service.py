...
from app_backend.domain.tasks import TaskDetail, TaskPublicationSummary
        ...
        *,
        ...
        if not dataset_actions.ingest_raw_data:
            raise service_error(
                ...
                message=(
                    "The active session cannot save simulation results into the target dataset."
                ),
            )
        ...
        if draft.design_id is not None:
            ...
            if design.redirect_design_id is not None:
                raise service_error(
                    ...
                    category="conflict",
                    message=(
                        f"Design {draft.design_id} was redirected to {design.redirect_design_id}."
                    ),
                    details={
...
def _build_publication_design_id(design_name: str) -> str:
    slug = "-".join(
        token
        for token in "".join(
            character.lower() if character.isalnum() else " " for character in design_name
        ).split()
        if len(token) > 0
    )
