...
from app_backend.services.simulation_result_explorer_selection import (
...
class SimulationResultExplorerQueryService:
    ...
    def resolve_selection(
        ...
        *,
        ...
        resolved_output_port = selection_request.output_port or int(
            context.default_selection["output_port"]
        )
        resolved_input_port = selection_request.input_port or int(
            context.default_selection["input_port"]
        )
        if (
            ...
            raise service_error(
                ...
                category="validation_error",
                message=(
                    "Requested trace selection ports are not available for this simulation result."
                ),
            )
    ...
    *,
    ...
    if task.kind == "post_processing" and task.post_processing_setup is not None:
        ...
        if selection is not None:
            family = (
                selection.trace_family if selection.trace_family in FAMILY_LABELS else "s_matrix"
            )
