...
from app_backend.domain.datasets import DatasetDetail
...
from app_backend.services.session_projection_service import (
...
from app_backend.services.session_service_contracts import (
        ...
        *,
        ...
        if parsed.scheme not in {"http", "https"} or parsed.hostname is None or parsed.port is None:
            ...
            return ServerTargetValidationResult(
                ...
                message=(
                    "The selected server target must be an http(s) origin with an explicit port."
                ),
            )
