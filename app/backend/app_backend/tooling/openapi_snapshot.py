...
def _build_api_schema_components() -> dict[str, Any]:
    ...
    for module_name in (
        "app_backend.api.schemas.circuit_definitions",
        "app_backend.api.schemas.datasets",
        "app_backend.api.schemas.errors",
        "app_backend.api.schemas.session",
        "app_backend.api.schemas.storage",
        "app_backend.api.schemas.tasks",
    ):
        module = import_module(module_name)
        for _, candidate in inspect.getmembers(module, inspect.isclass):
            if not issubclass(candidate, BaseModel) or candidate is BaseModel:
                continue
            if not candidate.__module__.startswith("app_backend.api.schemas."):
                continue
    ...
    return {str(name): schema for name, schema in raw_schemas.items() if isinstance(schema, dict)}
