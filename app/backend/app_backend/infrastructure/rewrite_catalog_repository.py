    ...
    def merge_design_scopes(
        ...
        if self._durable_publication_repository is not None:
            ...
            if result is not None:
                ...
                return result
        target_trace_ids = {
            row.trace_id for row in self._trace_summaries.get((dataset_id, target_design_id), ())
        }
        source_trace_rows = tuple(self._trace_summaries.get((dataset_id, source_design_id), ()))
        ...
        for key in list(self._characterization_result_details.keys()):
            ...
            if key_dataset_id == dataset_id and key_design_id == source_design_id:
                ...
                self._characterization_result_details[(dataset_id, target_design_id, result_id)] = (
        ...
        return (
            CharacterizationAnalysisRegistryRow(
                ...
                trace_compatibility=CharacterizationAnalysisTraceCompatibility(
                    ...
                    summary=(
                        f"{len(compatible_traces)} design traces are compatible with "
                        "admittance resonance extraction, but the current runtime "
