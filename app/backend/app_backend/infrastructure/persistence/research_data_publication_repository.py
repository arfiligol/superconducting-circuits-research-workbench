...
from app_backend.domain.tasks import SimulationSetup, TaskDetail
...
class SqliteResearchDataPublicationRepository:
    ...
    def list_designs(
        ...
        with self._session_factory() as session:
            ...
            traces_by_design: dict[str, list[RewritePublishedSimulationTraceRecord]] = defaultdict(
                list
            )
            ...
            for publication_row in publication_rows:
                ...
                design_row = _design_row_with_lifecycle(
                    ...
                    source_coverage=_build_source_coverage_from_trace_rows(design_trace_rows),
                    compare_readiness=_compare_readiness_for_trace_rows(design_trace_rows),
            ...
            return SimulationResultPublicationResult(
                ...
                design=DesignBrowseRow(
                    ...
                    compare_readiness=_compare_readiness_for(
                        _build_source_coverage(trace_summaries)
                    ),
...
def _build_requested_trace_publications(
    *,
    ...
    selections = tuple(
        ResultTraceSelection.from_trace_key(trace_key) for trace_key in unique_trace_keys
    )
...
def _resolve_requested_parameter_names(
    *,
    ...
    base_parameter = " ".join(
        part for part in (parameter_name or "").strip().split() if len(part) > 0
...
def _build_selection_suffixes(
    *,
    ...
    coordinates = [
        _decode_sweep_coordinates(setup, selection.sweep_index) for selection in selections
    ]
    varying_axes = [
        axis_index
