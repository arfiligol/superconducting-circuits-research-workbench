...
from app_backend.infrastructure.invitation_delivery import WorkspaceInvitationDeliveryService
from app_backend.infrastructure.persisted_characterization_runtime import (
...
from app_backend.infrastructure.rewrite_task_repository import PersistedRewriteTaskRepository
from app_backend.infrastructure.session_jwt_transport import SessionJwtTransport
from app_backend.infrastructure.task_execution_runtime import TaskExecutionRuntime
from app_backend.infrastructure.trace_store_axis_value_loader import (
    TraceStoreAxisValueLoader,
)
from app_backend.services.audit_log_service import AuditLogService
...
from app_backend.services.simulation_result_explorer_service import (
...
from app_backend.services.workspace_collaboration_service import WorkspaceCollaborationService
...
class NoopProcessorRuntimeRepository:
    ...
    def mark_task_running(self, *args: object, **kwargs: object) -> None:
        return None

    def acknowledge_cancellation(self, *args: object, **kwargs: object) -> None:
        return None

    def acknowledge_termination(self, *args: object, **kwargs: object) -> None:
        return None

    def mark_task_terminal(self, *args: object, **kwargs: object) -> None:
        return None
...
def get_app_state_repository() -> AppStateRepository:
    settings = get_settings()
    bootstrap_metadata_schema(settings.database_path)
    return AppStateRepository(create_metadata_session_factory(settings.database_path))
...
@lru_cache(maxsize=1)
def get_processor_runtime_repository() -> NoopProcessorRuntimeRepository:
    return NoopProcessorRuntimeRepository()
...
@lru_cache(maxsize=1)
def get_runner_result_publisher() -> RunnerResultPublisher:
    settings = get_settings()
    bootstrap_metadata_schema(settings.database_path)
    return RunnerResultPublisher(
        settings=settings,
        task_service=get_task_service(),
        storage_metadata_repository=get_storage_metadata_repository(),
        metadata_session_factory=create_metadata_session_factory(settings.database_path),
    )
...
def reset_runtime_state() -> None:
    ...
    get_runner_result_publisher.cache_clear()
