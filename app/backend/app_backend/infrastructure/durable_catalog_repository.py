...
from app_backend.domain.circuit_definitions import CircuitDefinitionRecord
...
from app_backend.domain.storage import TracePayloadRef
...
from app_backend.domain.trace_structures import (
...
from app_backend.infrastructure.persistence.trace_capability_store import (
    delete_trace_capabilities,
    load_trace_capability_map,
    replace_trace_capabilities,
    trace_capabilities_equal,
)
from app_backend.infrastructure.storage_reference_factory import build_metadata_record_ref
...
class DurableCatalogRepository:
    ...
    def merge_design_scopes(
        ...
        target_trace_ids = {
            row.trace_id for row in self.list_trace_metadata(dataset_id, target_design_id)
        }
        source_trace_ids = {
            row.trace_id for row in self.list_trace_metadata(dataset_id, source_design_id)
        }
        ...
        for record_family, count in published_result.reparented_counts.items():
            reparented_counts[record_family] = reparented_counts.get(record_family, 0) + count
    ...
    def list_designs(
        ...
        for design_id, trace_rows in traces_by_design.items():
            ...
            design_name = (
                ...
                else (existing.name if existing is not None else design_id)
            )
            lifecycle_state = (
    ...
    def _upsert_design_row(
        ...
        *,
        ...
        from app_backend.infrastructure.persistence.models import RewriteDatasetDesignRecord
...
def _nd_grid_numeric_payload_from_preview_payload(
    ...
    for axis_index, (declared_axis, raw_axis) in enumerate(zip(axes, raw_grid_axes, strict=True)):
...
def _axes_with_numeric_payload(
    ...
    if len(lengths) == len(axes):
        return tuple(
            replace(axis, length=length) for axis, length in zip(axes, lengths, strict=True)
        )
...
def _sample_indices(point_count: int, sample_limit: int) -> tuple[int, ...]:
    ...
    return tuple(
        dict.fromkeys(
            int(index) for index in np.linspace(0, point_count - 1, sample_limit, dtype=np.int64)
        )
    )
...
def _nd_grid_preview_payload_from_numeric_payload(
    ...
    for axis in raw_axes:
        ...
        axis_length = (
            len(axis_values) if isinstance(axis_values, list) else int(axis.get("length", 0))
        )
        axes_payload.append(
            {
                "name": str(axis.get("name", "")).strip(),
