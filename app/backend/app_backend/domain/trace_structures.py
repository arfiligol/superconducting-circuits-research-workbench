...
def build_axis_coordinate_digest(
    *,
    ...
    normalized_values = (
        [round(float(value), 12) for value in values] if values is not None else None
    )
    payload = {
        "name": axis_name,
        "unit": unit,
...
def derive_input_collection_payload(
    ...
    normalized_axes_by_trace = [
        tuple(_normalize_axis(axis) for axis in _trace_axes(trace)) for trace in traces
    ]
    shared_axes = _shared_axes(normalized_axes_by_trace)
    sweep_axes = tuple(
        axis.name for index, axis in enumerate(shared_axes) if index > 0 and axis.length > 1
    )
    axis_signatures = tuple(
        signature for trace in traces if (signature := _trace_axis_signature(trace)) is not None
    )
...
def _axes_share_semantic_identity(left: _NormalizedAxis, right: _NormalizedAxis) -> bool:
    return left.name == right.name and left.unit == right.unit and left.length == right.length
...
def _trace_axes(trace: object) -> tuple[TraceAxis | Mapping[str, object], ...]:
    ...
    if isinstance(raw_axes, Sequence) and not isinstance(raw_axes, str | bytes):
        return tuple(axis for axis in raw_axes if isinstance(axis, TraceAxis | Mapping))
    return ()
...
def _normalize_axis(axis: TraceAxis | Mapping[str, object]) -> _NormalizedAxis:
    ...
    values = (
        tuple(float(value) for value in raw_values if isinstance(value, int | float))
