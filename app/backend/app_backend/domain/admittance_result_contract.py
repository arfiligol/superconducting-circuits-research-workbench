...
class AdmittanceResultSurface:
    ...
    def derived_axis_values(self) -> tuple[int, ...]:
        ...
        if len(self.frequency_grid_by_member) > 0:
            mode_count = max(
                (len(row) for member_grid in self.frequency_grid_by_member for row in member_grid),
                default=0,
            )
...
def serialize_admittance_surface(
    ...
    return {
        ...
        "frequency_grid_by_member": [
            [[float(value) if value is not None else None for value in row] for row in member_grid]
            for member_grid in surface.frequency_grid_by_member
        ],
...
def parse_admittance_surface(
    ...
    return AdmittanceResultSurface(
        ...
        masked_input_indices_by_member=tuple(
            tuple(int(index) for index in member_indices if isinstance(index, int | float))
            for member_indices in masked_input_indices_payload
            if isinstance(member_indices, Sequence) and not isinstance(member_indices, str | bytes)
        ),
        diagnostics=tuple(
            CharacterizationDiagnostic(
...
def _table_cells_for_member(
    ...
    *,
    ...
    for mode_index in surface.derived_axis_values:
        rows.append([row[mode_index] if mode_index < len(row) else None for row in member_grid])
    return rows
...
def _parse_members(
    ...
    members = (
        ...
        if isinstance(members_payload, Sequence) and not isinstance(members_payload, str | bytes)
        else ()
    )
    if len(members) > 0:
        return members
    ...
    return [
        {
            ...
            "y_values": [row[mode_index] if mode_index < len(row) else None for row in member_grid],
            "mask": [mode_index >= len(row) or row[mode_index] is None for row in member_grid],
        }
        for mode_index in surface.derived_axis_values
    ]
