...
from app_backend.domain.runtime_contracts.tasking.routing import LaneName
...
class TaskControlRuntimeProjection:
    ...
    def terminal_state(self) -> TerminalTaskState | None:
        if self.runtime_state in TERMINAL_TASK_STATES:
            return cast(TerminalTaskState, self.runtime_state)
        return None
...
def build_task_retry_lineage(
    *,
    ...
    return TaskRetryLineage(
        ...
        source_terminal_state=cast(TerminalTaskState, source_task_state),
    )
...
def build_lane_processor_summaries(
    ...
    *,
    ...
    lanes: tuple[LaneName, ...] = tuple(
        cast(LaneName, lane) for lane in sorted({heartbeat.lane for heartbeat in heartbeats})
    )
    return tuple(
...
def _coerce_processor_state(value: object) -> ProcessorState:
    if value in {"idle", "running", "degraded", "draining", "offline"}:
        return cast(ProcessorState, value)
