...
def derive_characterization_analysis_ids(
    ...
    present = {
        capability.analysis_id for trace in traces for capability in trace.analysis_capabilities
    }
        ...
        projected_rows.append(
            CharacterizationAnalysisRegistryRow(
                ...
                required_config_fields=(
                    spec.required_config_fields if spec is not None else row.required_config_fields
                ),
                trace_compatibility=CharacterizationAnalysisTraceCompatibility(
...
def evaluate_characterization_analysis_scope(
    *,
    ...
    selected_scope = tuple(
        traces_by_id[trace_id] for trace_id in selected_trace_ids if trace_id in traces_by_id
    )
...
def _evaluate_trace_against_role(
    *,
    ...
    if spec.dataset_family_gate_mode == "advisory" and not _dataset_family_matches_spec(
        spec, dataset_family
    ):
    ...
    if len(role.accepted_source_kinds) > 0 and trace.source_kind not in role.accepted_source_kinds:
        blocking_reasons.append(
            _capability_reason(
                ...
                message=(
                    f"Requires {_format_token_list(role.accepted_source_kinds)} trace sources."
                ),
                evidence={
    ...
    if (
        ...
        blocking_reasons.append(
            _capability_reason(
                ...
                message=(
                    f"Requires {_format_token_list(role.accepted_representations)} representation."
                ),
                evidence={
...
def _deficit_summary(
    ...
    *,
    ...
    return f"{matched_trace_count} {scope} {noun} match so far, but {'; '.join(deficit_parts)}."
    ...
    *,
    ...
    if incompatible_count > 0:
        ...
        issues.append(
            ...
            f"{spec.label.lower()}" + (f" because {reason}" if reason is not None else ".")
        )
    if len(issues) == 0:
        return spec.unavailable_summary
...
def _dataset_family_matches_spec(
    ...
    return normalized_family in {
        _normalize_dataset_family(family) for family in spec.dataset_families
    }
...
def _dataset_family_is_hard_excluded(
    ...
    return spec.dataset_family_gate_mode == "hard" and not _dataset_family_matches_spec(
        spec, dataset_family
    )
