...
export class DesktopRuntimeSupervisor {
  ...
  private async startLocalImpl(
    ...
    await this.showStatus({
      ...
      message: state.auto_start_local_runtime
        ? "Launching the local backend and Julia Runner sidecars."
  ...
  private async startLocalSidecars(
    ...
    const backendEnv = buildBackendEnvironment(paths, ports, backendUrl, backendSecrets);
    ...
    const juliaExecutable = await findExecutable("julia", paths.binDir);
    if (juliaExecutable === null) {
      throw new Error("julia was not found in the packaged runtime or PATH.");
    }
    await this.spawnManagedProcess(
      "julia-runner",
      juliaExecutable,
      [
        "--project",
        paths.runnerProjectDir,
        "-e",
        `using SuperconductingCircuitsRunner; run_polling_runner(backend_url="${backendUrl}")`,
      ],
      {
        cwd: paths.runtimeRoot,
        env: buildProcessEnvironment(paths.binDir),
      },
    );
  }
...
async function resolveRuntimePaths(): Promise<RuntimePaths> {
  ...
  return {
    ...
    runnerProjectDir: join(runtimeRoot, "core", "julia", "SuperconductingCircuitsRunner"),
...
function buildBackendEnvironment(
  ...
  return {
    ...
    PYTHONPATH: `${paths.runtimeRoot}:${paths.backendDir}`,
    PYTHONUNBUFFERED: "1",
