"""Ideal symmetric two-junction small-signal SQUID admittance model.
...
def calculate_y11_imaginary(
    ...
    """Calculate the imaginary part of the ideal small-signal Y11 model.

    Model:
    Y11(jω) = 1 / (jω(L_sq + Ls1)) + j ω C / (1 - ω^2 Ls2 C)

    Where L_sq = L_jun / 2.0 for two identical junction inductances in parallel.

    Args:
        L_jun: Per-junction small-signal inductance in nH
