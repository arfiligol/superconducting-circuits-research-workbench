...
class MultiResonanceVectorFitter:
    ...
    def fit(
        ...
        *,
        ...
        frequency_span_hz = (
            (float(np.min(self.f)), float(np.max(self.f))) if restrict_to_input_span else None
        )
        extracted = self._extract_physics(min_q=min_q, frequency_span_hz=frequency_span_hz)
