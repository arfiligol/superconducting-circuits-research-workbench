using SuperconductingCircuitsCore
using Test

include("fixtures/minimal_component_library.jl")
using .MinimalComponentLibrary

const mm = 1e-3

function base_line_spec(; length_m=1.0mm, n_sections=4)
    return RLGCSpec(
        length_m=length_m,
        n_sections=n_sections,
        l_per_m_h=4.2e-7,
        c_per_m_f=1.7e-10,
    )
end

include("test_low_level_helpers.jl")
include("test_transmission_line_ladder.jl")
include("test_mtl_coupled_window.jl")
include("test_circuit_plan.jl")
include("test_endpoints.jl")
include("test_relations.jl")
include("test_parameters.jl")
include("test_engineering_graph.jl")
include("test_macro_dsl.jl")
include("test_compiler_skeleton.jl")
include("test_compiler_lumped_lowering.jl")
include("test_linear_modes.jl")
include("test_linear_response.jl")
include("test_hb_problem_spec.jl")
include("test_hbsolve_runner.jl")
include("test_result_extractors.jl")
include("test_sweeps.jl")
include("test_inspection.jl")
include("test_diagnostics.jl")
include("test_pluto_example_builders.jl")
include("test_intrinsic_interferometric_purcell_components.jl")
