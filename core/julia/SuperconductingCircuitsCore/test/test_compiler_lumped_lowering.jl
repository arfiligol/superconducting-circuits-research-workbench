...
@testset "lumped compiler lowers supported relations" begin
    ...
    @test length(compiled.netlist) == 6
    ...
    @test ("Lj1_drive_jj", "ext_drive", "0", :Lj1_drive_jj) in compiled.netlist
    ...
    @test compiled.component_map["lc"] == [1, 2, 3, 4, 6]
    ...
    @test compiled.component_values[:Lj1_drive_jj] == 6.0e-9
    ...
    @test (:josephson_junction, "drive_jj", (:pin, "lc", :signal), (:ground,)) in topology_relations
    @test topology_key(compiled) isa TopologyKey
    @test compiled.metadata[:netlist_row_count] == 6
