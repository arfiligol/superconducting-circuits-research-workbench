...
function add_readout_line!(
    ...
    if !isnothing(left_port_number)
        left_termination_ohm > 0 || _validation_error("left_termination_ohm must be positive.")
    ...
    if !isnothing(right_port_number)
        right_termination_ohm > 0 || _validation_error("right_termination_ohm must be positive.")
