...
runtime_require_path "$ROOT_DIR/app/backend/.venv" "cd app/backend && uv sync" "app:dev"
...
runtime_start_service "$PID_DIR" "$LOG_DIR" backend \
  bash -lc "cd '$ROOT_DIR' && export PYTHONPATH='$ROOT_DIR:$ROOT_DIR/app/backend' && exec uv run --project app/backend uvicorn src.app.main:app --host '$APP_HOST' --port '$APP_PORT'"
