...
function _validate_coupled_window_spec(spec::CoupledWindowSpec)
    spec.length_m > 0 || error("length_m must be positive.")
    spec.n_sections > 0 || error("n_sections must be positive.")
    spec.l11_per_m_h > 0 || error("l11_per_m_h must be positive.")
    spec.l22_per_m_h > 0 || error("l22_per_m_h must be positive.")
    spec.c1g_per_m_f > 0 || error("c1g_per_m_f must be positive.")
    spec.c2g_per_m_f > 0 || error("c2g_per_m_f must be positive.")
    spec.cm_per_m_f >= 0 || error("cm_per_m_f must be non-negative.")
    spec.r1_per_m_ohm >= 0 || error("r1_per_m_ohm must be non-negative.")
    spec.r2_per_m_ohm >= 0 || error("r2_per_m_ohm must be non-negative.")
    spec.g1_per_m_s >= 0 || error("g1_per_m_s must be non-negative.")
    spec.g2_per_m_s >= 0 || error("g2_per_m_s must be non-negative.")

    coupling_limit = sqrt(spec.l11_per_m_h * spec.l22_per_m_h)
    abs(spec.lm_per_m_h) <= coupling_limit ||
        error("abs(lm_per_m_h) must not exceed sqrt(l11_per_m_h * l22_per_m_h).")
end
