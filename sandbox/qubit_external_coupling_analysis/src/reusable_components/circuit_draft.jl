...
function LineSpan(start_m::Real, stop_m::Real)
    ...
    start_value >= 0 || error("LineSpan start_m must be non-negative.")
    stop_value > start_value || error("LineSpan stop_m must be greater than start_m.")

    return LineSpan(start_value, stop_value)
end
...
function CircuitDraft(name::AbstractString; ground_node::AbstractString="0")
    isempty(name) && error("CircuitDraft name must not be empty.")
...
function add_component!(
    ...
    isempty(name) && error("Component name must not be empty.")
...
function add_port_with_termination!(
    ...
    port_number > 0 || error("port_number must be positive.")
    resistance_ohm > 0 || error("resistance_ohm must be positive.")
...
function _register_pin_component!(draft::CircuitDraft, component)
    ...
    haskey(draft.pin_components, component_id) &&
        error("Component id '$(component_id)' is already registered in this draft.")
...
function pin_node(component::TransmissionLineInstance, pin::Symbol)
    ...
    error("TransmissionLineInstance supports only :left and :right pins.")
end
...
function pin_node(component::ReadoutLineComponent, pin::Symbol)
    ...
    error("ReadoutLineComponent supports only :left and :right pins.")
end
...
function pin_node(component::HalfWavePurcellFilterComponent, pin::Symbol)
    ...
    error("HalfWavePurcellFilterComponent supports only :left and :right pins.")
end
...
function pin_node(component::QuarterWaveResonatorComponent, pin::Symbol)
    ...
    error("QuarterWaveResonatorComponent supports only the :feed pin.")
end
...
function _resolve_pin_component(draft::CircuitDraft, component_ref)
    if component_ref isa AbstractString
        ...
        component === nothing && error("Unknown component id '$(component_ref)'.")
        return component
    end
    ...
    registered === nothing &&
        error("Component '$(component_id)' is not registered in this draft.")
    return registered
end
...
function apply_series_chain!(draft::CircuitDraft, components...)
    length(components) >= 2 ||
        error("apply_series_chain! requires at least two components.")
...
function add_quarter_wave_resonator_component!(
    ...
    coupling_cap_f > 0 || error("coupling_cap_f must be positive.")
    boundary in (:open, :short) || error("boundary must be :open or :short.")
...
function _register_internal_transmission_line!(
    ...
    haskey(draft.transmission_lines, line_id) &&
        error("Internal transmission line id '$(line_id)' is already registered.")
...
function add_transmission_line!(
    ...
    haskey(draft.pin_components, line_id) &&
        error("Component id '$(line_id)' is already registered in this draft.")
    haskey(draft.transmission_lines, line_id) &&
        error("Transmission line id '$(line_id)' is already registered in this draft.")
...
function apply_coupled_window!(
    ...
    any(window -> window.id == placement_id, draft.coupled_windows) &&
        error("Coupled window id '$(placement_id)' is already in use.")
    ...
    line_a_instance.id != line_b_instance.id ||
        error("A coupled window must connect two different transmission lines.")
    ...
    span_length(span_a) ≈ span_length(span_b) ||
        error("Coupled spans on line A and line B must have the same physical length.")

    span_length(span_a) ≈ spec.length_m ||
        error("Coupled-window spec length_m must match the requested span length.")
...
function finalize_to_josephson_netlist(
    ...
    for line_id in draft.line_order
        ...
        for chunk in chunks
            ...
            elseif chunk.kind == :coupled
                ...
                error("Unknown chunk kind '$(chunk.kind)'.")
            end
        end
    end
    ...
    for placement in draft.coupled_windows
        ...
        endpoints_by_line === nothing &&
            error("Internal error: missing endpoints for coupled window '$(placement.id)'.")
        ...
        endpoints_a === nothing &&
            error("Internal error: missing line-A endpoints for coupled window '$(placement.id)'.")
        endpoints_b === nothing &&
            error("Internal error: missing line-B endpoints for coupled window '$(placement.id)'.")
...
function _resolve_line_instance(draft::CircuitDraft, line_ref)
    ...
    line === nothing && error("Unknown transmission line id '$(line_id)'.")
    return line
end
...
function _approximate_section_count(spec::RLGCSpec, chunk_length_m::Float64)
    chunk_length_m > 0 || error("Chunk length must be positive.")
