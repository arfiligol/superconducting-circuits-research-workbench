...
function _validate_rlgc_spec(spec::RLGCSpec)
    ...
    spec.l_per_m_h > 0 || error("l_per_m_h must be positive.")
    spec.c_per_m_f > 0 || error("c_per_m_f must be positive.")
    spec.r_per_m_ohm >= 0 || error("r_per_m_ohm must be non-negative.")
    spec.g_per_m_s >= 0 || error("g_per_m_s must be non-negative.")
end
...
function _add_series_section!(circuit, prefix, idx, left_node, right_node, values)
    ...
    error("Each distributed section must include positive series inductance.")
end
