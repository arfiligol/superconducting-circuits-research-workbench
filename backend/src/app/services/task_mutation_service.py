...
class TaskMutationRepository(Protocol):
...
class TaskMutationDatasetRepository(Protocol):
...
class TaskMutationCircuitDefinitionRepository(Protocol):
    def get_circuit_definition(self, definition_id: str) -> object | None: ...


class TaskMutationSessionRepository(Protocol):
    def get_session_state(self) -> SessionState: ...


class TaskMutationAuditRepository(Protocol):
    def append(self, record: AuditRecord) -> None: ...


class TaskMutationQueueDispatcher(Protocol):
    def enqueue_submitted_task(self, task: TaskDetail) -> TaskDispatchReceipt: ...


class TaskMutationHost(Protocol):
...
class TaskMutationService:
    def __init__(
        ...
        session_repository: TaskMutationSessionRepository,
        dataset_repository: TaskMutationDatasetRepository,
        circuit_definition_repository: TaskMutationCircuitDefinitionRepository,
        authorization_service: AuthorizationService | None = None,
        audit_repository: TaskMutationAuditRepository | None = None,
        queue_dispatcher: TaskMutationQueueDispatcher | None = None,
    ) -> None:
    ...
    def submit_task(self, draft: TaskSubmissionDraft) -> int:
        ...
        self._enqueue_submitted_task_if_supported(created_detail, runtime_mode=session.runtime_mode)
        return created_task.task_id

    def cancel_task(self, task_id: int, *, host: TaskMutationHost) -> int:
    ...
    def terminate_task(self, task_id: int, *, host: TaskMutationHost) -> int:
        task = host.get_task(task_id)
        session = self._session_repository.get_session_state()
        self._authorize_task_action(
            session,
    ...
    def retry_task(self, task_id: int, *, host: TaskMutationHost) -> int:
