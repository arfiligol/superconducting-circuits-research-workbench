...
class TraceCollectionService:
    ...
    def derive_data_collection_review_from_trace_details(
        ...
        readiness_state = (
            "ready"
            if len(runnable) > 0
            else ("inspect_only" if len(blocked) > 0 else "blocked")
        )
...
def _member_label(detail: TraceDetail) -> str:
    return (
        f"{detail.source_kind.replace('_', ' ')} · "
        f"{detail.parameter} ({detail.representation})"
    )
...
def _member_provenance_summary(detail: TraceDetail) -> str:
    ...
    if isinstance(preview_payload, dict):
        ...
        if isinstance(raw_summary, str) and len(raw_summary) > 0:
            return raw_summary
    return (
        f"{detail.source_kind.replace('_', ' ')} {detail.parameter} "
        f"{detail.representation} trace."
    )
