...
| --- | --- | --- | --- |
| `work-lane-orchestration` | Recommended | 任務需要主 thread 統整、多 surface vertical slice、subagent work lanes、SoT loading、跨邊界驗收 | [Work Lane Orchestration](work-lane-orchestration.mdx) |
| `write-pluto-notebook` | Recommended | 任務需要建立、修改、轉換或審查 Pluto.jl reactive notebook，尤其是 `.jl` notebook、PlutoUI / `@bind`、package environment、cell dependency 或 duplicate global definition 問題 | [Write Pluto Notebook](write-pluto-notebook.mdx) |
...
2. 更新本頁 `Catalog`。
3. 若新增頁面，確認 Starlight sidebar 與 route checks 能找到新頁面。
