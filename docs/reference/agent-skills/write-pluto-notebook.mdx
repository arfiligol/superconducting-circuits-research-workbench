...
owner: docs-team
audience: contributor
scope: Cross-tool Skill template and synchronization rules for Pluto.jl reactive notebook authoring
version: v0.1.0
...
`write-pluto-notebook` is the Pluto.jl notebook authoring Skill for coding agents. It requires agents to treat Pluto notebooks as reactive Julia programs and interactive teaching documents, rather than as Jupyter-style transcripts that rely on manual execution sequences.

This template can be synced to Codex App, Claude Code, Gemini or other tools that support reusable instruction. If the tool supports bundled resources, it should synchronize the Pluto behavior reference, featured notebook pattern reference and duplicate global definition checker; if it does not support it, at least retain the hard rules in the `SKILL.md` template.

<Aside type="caution" title="Reactivity, PlutoUI, and Markdown are first-class">

Pluto's dependency graph relies on syntax analysis. The synchronized Skill must retain the "one global owner cell per name" rule, and must also regard PlutoUI, `@bind`, `WideCell`, Markdown admonitions, `details`, `TableOfContents`, `combine`, `confirm`, etc. as core specifications for Pluto notebook authoring, rather than additional style recommendations.

</Aside>

## Source Check

This template is compiled based on Pluto official documents and featured notebook corpus reviewed on 2026-05-30:
...
No Pluto-specific Codex Skill provided by Pluto official or OpenAI officials was found. This template is a project-provided Skill.

## Sync Steps

1. Create a reusable instruction, skill or context entry that is natively supported by the target agent.
2. Use `write-pluto-notebook` as the canonical name.
3. Keep the trigger description and ensure that `.jl` Pluto notebook, PlutoUI, `@bind`, Markdown admonitions, teaching callouts, PlotlyJS/`WideCell`, reactivity debugging, package environment setup, Pluto featured notebook style, and featured-notebook-style teaching material will trigger this Skill.
4. Synchronize the `SKILL.md` template below to the skill body.
5. If the target tool supports resources, add:
- `references/pluto-behavior.md`: Pluto official action summary and source link.
- `references/featured-notebook-patterns.md`: authoring patterns for `JuliaPluto/featured` and `PlutoUI.jl` corpus, including Markdown admonition usage summary.
- `scripts/check_pluto_reactivity.py`: raw `.jl` Pluto notebook of duplicate global definition and Plotly/WideCell heuristic checker.
