...
Python Notebook is the routine client for the accepted installable circuit runtime and is also a Python-native data-analysis, external-result, quantum-modeling, dynamics/pulse, and inspection surface. It may assemble a visible generic `CircuitPlan`, declare the consumer-owned bindings/reduction/objective/Gates/variables/optimizer on `CircuitSim`, and call explicit `evaluate`, `optimize`, or `analyze` actions. See [Circuit Runtime / Python Consumer](../../research-contracts/circuit-runtime-python-consumer.md).

Each `evaluate` starts one Julia process for the complete evaluation; each `optimize` starts one Julia process for the complete search. `analyze` is pure Python over sealed evidence. No candidate or frequency point calls back into Python.
