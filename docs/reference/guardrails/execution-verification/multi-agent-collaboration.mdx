...
| Product Async Contracts | `docs/reference/architecture/`, `app/backend/app_backend/domain/runtime_contracts/`, `app/backend/`, Backend API contracts | simulation request contracts, runner task envelopes, runner result manifest, result-view API, OpenAPI alignment |
