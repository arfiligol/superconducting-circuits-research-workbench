...
<Aside type="caution" title="No partial netlist reuse">
...
See [HB Simulation Intent](hb-simulation-intent.mdx) for the source-of-truth simulation-intent contract.
...
Matrix meaning and Maxwell-to-physical lowering are canonical in
...
this page owns only compiler behavior.

Port/wave semantics are canonical in [SCQ_Design: Port Reference Impedance Semantics](https://github.com/arfiligol/SCQ_Design/blob/main/docs/knowledge/simulation/port-reference-impedance-semantics.qmd).
...
- one `P<index>` row and one colocated `R_port_<index>` row for every declared
  external port; the resistor participates in returned raw Z and also defines
  the current JosephsonCircuits wave reference;
