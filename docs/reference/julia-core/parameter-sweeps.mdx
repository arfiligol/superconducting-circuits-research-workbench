...
Canonical Josephson and SQUID semantics:
...
Current Core sweeps may bind the implemented `josephson_inductance` of a fixed single-junction topology. `SQUID`, `junction_Ic`, and `external_flux` axes below are conceptual classification examples for a future owning component library; no dc-SQUID or external-flux lowering exists today.
...
| `component_type = LinearInductor / SQUID` | structural | Conceptual component-library axis; no current Core SQUID API |
...
| `josephson_inductance` | numeric if junction topology unchanged | Changes the implemented nonlinear `Lj...` binding only |
| `external_flux` | numeric or drive-like | Conceptual SQUID axis; no current Core lowering |
...
- a future component-library type changes from linear inductor to SQUID;
...
| `josephson_inductance` | `JosephsonJunction` relation | `NumericParameter` if junction topology is unchanged | emitted `Lj...` structure must not change |
| `external_flux` | Future Component Library or Plan Builder | `NumericParameter` or `DriveParameter` | conceptual until SQUID lowering exists |
...
| `window_length_um` | generated line boundaries and `MTLCoupledRLGCSpec.length_m` | `StructuralParameter` |
...
flux_bias
  ...
  -> NumericParameter or DriveParameter in a future owning component library
```
...
external_flux
  Conceptual until a component library owns SQUID lowering
  Then NumericParameter / DriveParameter if SQUID topology is fixed
  Then StructuralParameter if it selects a different component model or junction topology
```
...
The `flux_phi0` names in these patterns assume a future plan builder that owns dc-SQUID and external-flux lowering; they are not accepted by the current Core `JosephsonJunction` API.
