...
<Aside type="note" title="Pump-off is still a real source slot">
...
<Aside type="caution" title="Returned S and Z are different observable layers">

For the current JosephsonCircuits target, `returnS` constructs Kurokawa power
waves while `returnZ` reports the complete solved network, including the
required colocated port resistors. Workbench Y helpers invert returned Z; they
do not expose a native solver Y family. Do not run a generic `ZtoS` conversion
on returned raw Z. The exact reconstruction and mode limits are canonical in
SCQ_Design [Port Reference Impedance Semantics](https://github.com/arfiligol/SCQ_Design/blob/main/docs/knowledge/simulation/port-reference-impedance-semantics.qmd).
...
## Product Runner Exclusions

The Product Runner payload excludes controls that require observable, storage,
or provenance contracts outside this page. Excluded controls must fail clearly in
product Runner execution unless an explicit Julia Core contract owns them.

| Control | Contract boundary | Expected failure behavior |
| --- | --- | --- |
| `returnSnoise` | Noise observables require an explicit noise result schema and canonical storage mapping. | reject in Runner payload |
| `returnnodeflux` / `returnvoltage` | Node-resolved outputs require compiled node provenance and a storage schema. | reject in Runner payload |
| adjoint output flags | Adjoint result families require explicit observable request types. | reject in Runner payload |
| sensitivity flags | Sensitivity results require stable provenance and owned result contracts. | reject in Runner payload |
| custom `factorization` | Runtime object injection is not a product task payload contract. | reject in Runner payload |
...
- circuit-family forbidden-frequency validation is a Julia Core validation concern; when a circuit family declares forbidden regions, Runner treats violations as Core validation errors.
...
- [HB Simulation Intent](hb-simulation-intent.mdx)
