...
Reusable harmonic-balance theory, periodic-steady-state assumptions, truncation meaning, and evidence gates are owned by the SCQ_Design [Harmonic Balance: Periodic Steady State and Mode Semantics](https://github.com/arfiligol/SCQ_Design/blob/main/docs/knowledge/numerical-methods/harmonic-balance-periodic-steady-state.qmd) node. This page owns only the Julia Core HB-intent API and its compile/runtime handoff.
...
<Aside type="caution" title="Runner does not own HB semantics">
...
Reusable wave-reference, explicit-shunt, raw-Z, and derived-Y semantics are
canonical in SCQ_Design [Port Reference Impedance Semantics](https://github.com/arfiligol/SCQ_Design/blob/main/docs/knowledge/simulation/port-reference-impedance-semantics.qmd).
This page owns only the Julia Core declaration and lowering contract.
...
| `resistance_ohm` | positive resistance used by the current compiler to emit the colocated JosephsonCircuits port-impedance resistor; it defines the solver wave reference and participates in returned raw Z |
...
- the current target requires exactly one colocated resistor per port; do not
  model a separate retained physical load by silently reusing this field;
...
- circuit-family forbidden-frequency validation is a Julia Core validation concern.
...
<Aside type="caution" title="Do not create probe sources">
...
- [JosephsonCircuits hbsolve Controls](josephsoncircuits-hbsolve-controls.mdx)
