...
!!! info "Surface Boundary"
    Header 負責唯一可見的 shell identity、`Runtime Mode`、`Active Workspace`、`Active Dataset`、`Task Execution`、Runner runtime status summary、account surface 與 app-level preferences。
...
!!! warning "Global Context Lives Behind Compact Triggers"
    `Runtime Mode`、`Active Workspace`、`Active Dataset` 與 `Task Execution` 是 shared shell 的 global context。
...
!!! tip "Compact Trigger, Selected Detail Elsewhere"
    ...
    實際的 workspace switch、dataset switch、recent task rows、account preference 與 debug disclosure，應集中在右側 `Shell-Side Panel`；Runner status detail 與 long task history 應進入 `/tasks` 或 developer disclosure。

!!! tip "Read With Task Management"
    Header 負責「從哪裡切換 active workspace、切換 dataset、打開 task execution summary、看 Runner runtime 狀態、開啟 user menu」。
    task row 內 `Attach`、`Cancel`、`Terminate`、`Retry` 的行為語意，則由 [Task Management](../shared-workflow/task-management.md) 定義。
...
| Global Context Cluster | `Runtime Mode`、`Active Workspace`、`Active Dataset`、`Task Execution` 的 compact triggers / chips |
...
| Concern | Required behavior |
|---|---|
| Role | shared shell 的 compact management entry，承接 workspace、dataset 與 recent task detail |
| Top row | 顯示 3 至 4 個 compact section triggers：`Runtime Mode`、`Active Workspace`、`Active Dataset`、`Task Execution` |
...
| Runner rule | Runner runtime 只在 task execution trigger 顯示 compact health badge；Runner detail 屬於 `/tasks` 或 developer disclosure |
...
!!! tip "Selected-section detail only"
    ...
    不要把 runtime mode、workspace、dataset、task execution、Runner runtime 五段內容整包常駐展開。
...
| 2 | `Active Workspace` | 決定 dataset list、task visibility 與 capability context |
| 3 | `Active Dataset` | 決定 workflow pages 的預設 dataset scope |
| 4 | `Task Execution` | 顯示目前 workspace / mode 中的 task activity |
...
=== "Runtime Mode Trigger"
    ...
    | unsafe-context handling | 若切 mode 會清掉 dirty draft、attached task 或 task context，必須先要求確認 |
...
=== "Active Workspace Trigger"
    ...
    | propagation | 切換後必須同步更新 active dataset、task visibility、role / capabilities |
...
=== "Active Dataset Trigger"
    ...
    | propagation | 切換後必須同步更新 Dashboard、Raw Data、Simulation Workbench、Analysis Workbench |
...
=== "Task Execution Trigger"
    ...
    | Element | Required behavior |
    |---|---|
    | task button / badge | 顯示目前可見 active tasks 數量 |
    | open behavior | 點擊後打開右側 shell-side panel 的 `Global Context` task execution section |
    | task section | 展示少量 recent task rows 與 filter (`Workspace` / `Mine`)；非 selected sections 只留 compact trigger 摘要 |
    | mode behavior | local mode 顯示 local runtime tasks；online mode 顯示 workspace-visible shared tasks |
    | Runner runtime summary | Header / Global Context 只保留 compact health badge；各 Runner 的 `idle / running / degraded / draining / offline` detail 進入 `/tasks` 或 developer disclosure |
    | liveness wording | `idle` 代表 Runner alive and available；`offline` 只代表 unavailable，不得把 merely idle Runner 或被動 heartbeat 稀疏的 Runner 誤標成 `offline` |
...
|---|---|
| workspace changed | 重新繫結 active dataset、刷新 task visibility、更新 role 與 user menu capabilities |
| dataset changed | 更新 dataset chip，通知 workflow pages 重讀 dataset-bound state |
| workspace caused task detachment | 顯示 detached notice，並將 task execution panel 保持可重新 attach |
...
| Runtime mode is the outer shell boundary | `Runtime Mode` 先決定 local / online authority，再往下決定 workspace、dataset 與 task context |
| Workspace is top-level shell context | `Active Workspace` 優先於 `Active Dataset`，因為 dataset list、task visibility 與 capabilities 都依賴它 |
| Task execution is globally reachable | 不論目前在哪一頁，都能從 Header 打開 `Task Execution` |
| Runner runtime summary is runtime-driven | Header 顯示的 compact Runner badge 必須來自 runtime summary，不可由 UI 推測 |
| Header is summary-first | Header 可以提示 dirty / attached / stale，但不應攤平大型管理 UI |
| Shell-side panel owns selected detail | workspace switch、dataset switch、recent task rows 與 account detail 應集中在右側 panel；Runner detail 與 long history 屬於 `/tasks` |
| Account is preference-first | account panel 優先承擔 personal/app preference，不承擔 workspace 與 collaboration 管理面 |
| Responsive collapse | 窄螢幕可縮成 icon + chips，但仍必須保留 dataset、task execution 與 user menu trigger |
