...
| Dataset / Design / Trace Model | data-formats + backend | [Data Formats / Dataset / Design / Trace Schema](../data-formats/dataset-record.md), [App / Backend / Datasets & Results](../app/backend/datasets-results.md) | dashboard, raw data browser, data ingestion, simulation publication, characterization, CLI datasets | `active_dataset` binds dataset；`DesignScope` is dataset-local lifecycle resource；target selection uses explicit `design_id` or create-new intent；parameter sweeps persist as canonical ND traces, not point-row authority |
