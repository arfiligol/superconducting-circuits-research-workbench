...
| Task lifecycle | Backend task table + runner API | [Tasks & Execution](../app/backend/tasks-execution.md) | `partial` | DB-backed claim/complete path exists; only explicit debug smoke tasks write smoke output |
| Julia Runner | `core/julia/SuperconductingCircuitsRunner` | [Julia Runner Compute Plane](julia-runner-compute-plane.md) | `partial` | explicit smoke task writes local Zarr and manifest; real task dispatch fails loudly until implemented |
...
| Canonical TraceStore | `data/trace_store/.../<batch_id>.zarr` | [TraceStore Zarr](trace-store-zarr.md) | `partial` | backend validates readable staging Zarr and publishes into canonical store |
...
| User-facing command workflow | none | current architecture | `removed` | helper automation belongs in `scripts/` only |
| Retired Python UI runtime | none | current architecture | `removed` | not an active dependency or entrypoint |
| Retired local queue workers | none | current architecture | `removed` | local runtime starts frontend, Python Backend, and Julia Runner |
