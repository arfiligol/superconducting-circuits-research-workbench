...
<Aside type="note" title="撰寫原則">

- 所有內容遵循 [Physics Concepts Guardrail](../../reference/guardrails/documentation-design/explanation-physics.mdx)
- 每篇頁面標註節點類型（Principle / Model / Method / Device）

</Aside>

- 工具操作步驟不放在此處，請參考 [Workflows](../../workflows/index.mdx)
...
## G. Design & Simulation (原理面)
> 模擬工具背後的物理原理與數值方法。工具操作步驟請參考 Workflows。
...
## H. Cryogenic RF Engineering (原理面)
> 低溫微波量測鏈的物理原理。儀器操作 SOP 請參考 Workflows。
...
## I. Experiments & Protocols (原理面)
> 實驗協議背後的物理原理。量測 SOP 請參考 Workflows。
...
- [Workflows](../../workflows/index.mdx) — 工具操作、資料匯入與端到端流程
- [Reference](../../reference/index.mdx) — contracts、資料格式與 API 入口
- [Notebook Interface](../../reference/notebooks/index.md) — Pluto 與 Python notebook 角色邊界
