...
- **前置閱讀**：[超導體的宏觀波函數](macroscopic-wavefunction.mdx)——序參量 $\Psi(\mathbf{r}) = |\Psi| e^{i\theta}$ 的物理意義

<details class="docs-disclosure docs-disclosure--info">
<summary>若要補背景，請先讀</summary>

- [超導體的宏觀波函數](macroscopic-wavefunction.mdx)：理解為何超導體可用單一波函數描述
...
<Aside type="note" title="規範不變性的物理含義">
...
<Aside type="tip" title="為什麼這個組合是規範不變的？">
...
- 在含 Josephson 接面的迴路中，磁通量子化條件會修正為包含接面相位差的形式（見 [SQUID 控制非線性電感](squid-controls-nonlinear-inductance.mdx)）。
...
- 前置：[超導體的宏觀波函數](macroscopic-wavefunction.mdx)
- 下一步：[為什麼非線性會讓能階間距不一致？](why-nonlinearity-makes-unequal-level-spacing.mdx)
- 任務導向：[Flux Dependence Analysis](../../workflows/analysis-fitting/flux-analysis.md)
