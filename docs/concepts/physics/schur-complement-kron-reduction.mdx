...
<Aside type="caution" title="同一個核心可重用">
...
<Aside type="caution" title="目前是 Port-Level，不是 Nodal-Level">

目前研究流程的 post-processing 作用在模擬回傳的 port-space `Y(ω)`。
不是完整內部節點（nodal）矩陣，因此無法直接做 arbitrary internal-node elimination。

</Aside>

<Aside type="note" title="可比對 HFSS 的脈絡">
