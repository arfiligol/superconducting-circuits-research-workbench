...
<Aside type="note" title="研究筆記（工作中）">

本頁記錄目前研究 notebook 可重現、且與正式 Reference 契約一致的流程。
...
- [Schur Complement 與 Kron Reduction（Concept）](./schur-complement-kron-reduction/)
- [Analysis Result Data Format（HFSS comparable 欄位語意）](../reference/data-formats/analysis-result/)
- [Physics Symbol Glossary（符號對照）](./symbol-glossary/)
...
<Aside type="note" title="Auto alpha/beta 的正式規則">
...
<Aside type="caution" title="Raw S / PTC 邊界（Reference 契約）">
