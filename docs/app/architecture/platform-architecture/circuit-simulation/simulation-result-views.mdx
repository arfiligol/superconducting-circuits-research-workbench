...
<Aside type="caution" title="PTC is target architecture, not current Runner support">

The current Runner publishes solver-native S traces and does not produce a
persisted PTC bundle; post-processing execution remains fail-fast, and new
simulation submissions with `ptc.enabled=true` are rejected before persistence.
The PTC flow below describes the intended result-view boundary. Request
configuration must not be presented as artifact availability. See SCQ_Design [Port Reference
...
The target architecture separates three result nodes:
