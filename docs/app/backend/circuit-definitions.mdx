...
simulation 與 analysis run 不屬於本頁責任。

</Aside>

<Aside type="tip" title="Primary Consumers">

主要消費者是 [Schemas](../frontend/definition/schemas.mdx)、[Schema Editor](../frontend/definition/schema-editor.mdx) 與 [Circuit Simulation](../frontend/simulation-workbench/circuit-simulation.md)。
...
- catalog、editor、simulation 與 task binding 一律以 UUIDv4-only `definition_id` 運作
...
| **Reuse** | 同一 definition detail 必須可供 schema editor 與 circuit simulation 共用。 |
