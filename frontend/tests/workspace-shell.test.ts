...
describe("workspace shell source contracts", () => {
  ...
  it("rebuilds runtime mode into selectable cards with inline target and refresh affordances", () => {
    ...
    expect(statusStripSource).toContain("Refresh Local Space session.");
    expect(statusStripSource).not.toContain("does not call online auth refresh");
  ...
  it("combines queue and worker runtime context into one shell surface", () => {
    ...
    expect(statusStripSource).toContain("Open Tasks for lane detail.");
    expect(statusStripSource).not.toContain("Worker lanes");
    expect(statusStripSource).not.toContain('label="Pending"');
    expect(statusStripSource).not.toContain("laneSummary.idleProcessors");
